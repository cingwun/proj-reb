...
<form action="{{URL::route('spa.admin.share.gallery.write')}}" method="post">
    ...
    <div class="form-group">
        <label for="language">這張圖片用於: </label>
        <label class="radio-inline">
            <input type="radio" name="language" value="tw" id="optionsRadiosInline" checked {{(Arr::get($data, 'language')=='tw') ? 'checked' : ''}}> 繁體版網頁
        </label>
        <label class="radio-inline">
            <input type="radio" name="language" value="cn" id="optionsRadiosInline" {{(Arr::get($data, 'language')=='cn') ? 'checked' : ''}}> 簡體版網頁
        </label>
    </div>
